...
- `weight` — stored in kg, always. **Omitted entirely** (not `0`, not `null` — the key is absent) on sets logged for a bodyweight exercise (`exercise.equipment` includes `"bodyweight"`, see "Exercise entry" above) — there's no meaningful load to record, and the Log screen doesn't even show a weight field for these. `js/utils.js`'s `formatSetRow()` is the one place that renders a set row (shared by the Log screen's active session and History's log detail sheet) and checks for the field's presence before showing a weight value. A kg/lbs display preference (Settings screen, `js/data.js`'s `loadUnit()`/`saveUnit()`) converts at the entry/display boundary only (`js/utils.js`'s `convertKgToUnit()`/`convertUnitToKg()`) — the stored value here is unaffected by which unit was used to enter or view it
