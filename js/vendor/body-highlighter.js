// Vendored + forked from body-highlighter v3.0.2 (MIT):
// https://github.com/lahaxearnaud/body-highlighter
//
// Reimplemented here in readable form rather than hand-patched from the
// minified CDN bundle, with two additions the original doesn't support:
//
//   1. A `gender: 'male' | 'female'` option (default 'male') — paints
//      MALE_ANTERIOR/MALE_POSTERIOR (below, extracted verbatim from the
//      original bundle — the existing model, unchanged) or
//      FEMALE_ANTERIOR/FEMALE_POSTERIOR (body-highlighter-female-data.js,
//      adapted from a different MIT-licensed project).
//   2. <path> element support alongside the original's <polygon>-only
//      rendering, since the female geometry is curved path data
//      (`paths: [d, d, ...]`) rather than straight-line points
//      (`svgPoints: [points, points, ...]`).
//
// Everything else — frequency-based colouring (highlightedColors indexed
// by how many synthetic entries in `data` mention a muscle), click
// handling, update()/destroy() lifecycle — is unchanged from the
// original's behaviour. See docs/decisions.md "Add a female body model
// option" for why this fork exists and how the female geometry's
// coordinate system was adapted to fit here without touching its own
// path data (see FEMALE_VIEW_TRANSFORMS below).
//
// Simplifications made for this fork, since this app only ever calls it
// with its own already-canonical region ids (never natural-language
// muscle names): the original's alias-normalization table (mapping e.g.
// "Pectoralis Major" -> "chest") is dropped — js/muscle-taxonomy.js
// already does that translation before anything reaches this file.

import { FEMALE_ANTERIOR, FEMALE_POSTERIOR } from './body-highlighter-female-data.js';

const SVG_NS = 'http://www.w3.org/2000/svg';
const DEFAULT_BODY_COLOR = '#B6BDC3';
const DEFAULT_HIGHLIGHTED_COLORS = ['#81b1d9', '#277abf'];
const DEFAULT_WRAPPER_CLASS = 'rbh-wrapper';

// Canonical region ids, matching js/muscle-taxonomy.js's MUSCLE_TO_REGIONS targets.
const MUSCLE_ID = {
  TRAPEZIUS: "trapezius",
...
// Existing male geometry, extracted verbatim from body-highlighter v3.0.2 —
// unchanged from what the app already used via the CDN import.
export const MALE_ANTERIOR = [
  {
    muscle: MUSCLE_ID.CHEST,
    svgPoints: [
  ...
  {
    muscle: MUSCLE_ID.FRONT_DELTOIDS,
    svgPoints: [
...
export const MALE_POSTERIOR = [
  ...
  {
    muscle: MUSCLE_ID.BACK_DELTOIDS,
    svgPoints: [
...
// One entry per canonical region id, zeroed out — cloned fresh on every
// render so per-muscle stats never leak between renders. Deliberately
// covers every id in MUSCLE_ID regardless of which gender/view is active
// (both models draw from the same shared vocabulary) — aggregateStats()
// below ignores any id passed in `data` that isn't a key here, so this
// only needs to be a superset, never an exact match per view.
const EMPTY_STATS = Object.fromEntries(
  Object.values(MUSCLE_ID).map((id) => [id, { exercises: [], frequency: 0 }])
);

// The female source data (body-highlighter-female-data.js) is one shared
// coordinate plane split into front/back halves — each view's own
// original viewBox (as used by the source project's SVG wrapper),
// [x, y, width, height]. Used to compute a transform that fits each
// view's paths into this library's fixed "0 0 100 200" box without
// touching the source path coordinates at all. Confirmed live: a female
// back-view path started at x=1096, squarely inside that view's offset
// range, before this transform is applied.
const FEMALE_SOURCE_VIEWBOX = {
  anterior: { x: -50, y: -40, width: 734, height: 1538 },
  posterior: { x: 756, y: 0, width: 774, height: 1448 },
};
const TARGET_VIEWBOX_WIDTH = 100;
const TARGET_VIEWBOX_HEIGHT = 200;

function femaleViewTransform(view) {
  const box = FEMALE_SOURCE_VIEWBOX[view];
  const scaleX = TARGET_VIEWBOX_WIDTH / box.width;
  const scaleY = TARGET_VIEWBOX_HEIGHT / box.height;
  // Male's own plane happens to scale by an exact uniform 7.24 on both
  // axes (724/100 = 1448/200); female's is close but not identical
  // (~7.34 horizontally vs. ~7.69 vertically) — a small, expected
  // non-uniform stretch, not a bug. See docs/decisions.md.
  const translateX = -box.x * scaleX;
  const translateY = -box.y * scaleY;
  return `translate(${translateX},${translateY}) scale(${scaleX},${scaleY})`;
}

const FEMALE_VIEW_TRANSFORMS = {
  anterior: femaleViewTransform('anterior'),
  posterior: femaleViewTransform('posterior'),
};

// Rolls the `data` array (synthetic {name, muscles, frequency} entries —
// see js/heatmap.js's buildLibraryData()) up into per-muscle stats.
function aggregateStats(data) {
  const stats = structuredClone(EMPTY_STATS);
  data.forEach((entry) => {
    (entry.muscles || []).forEach((muscleId) => {
      if (!stats[muscleId]) return; // unknown id — ignore rather than throw
      stats[muscleId].exercises.push(entry.name);
      stats[muscleId].frequency += entry.frequency || 1;
    });
  });
  return stats;
}

// highlightedColors is indexed by (frequency - 1), clamped to the
// array's own length — handing it a long interpolated gradient (see
// js/heatmap.js's buildGradient()) rather than requiring exactly N
// colours is what makes the continuous heat gradient possible at all.
function colorForMuscle(stats, highlightedColors, muscleId) {
  const frequency = stats[muscleId]?.frequency;
  if (!frequency) return null;
  return highlightedColors[Math.min(highlightedColors.length - 1, frequency - 1)];
}

// One <polygon> (male, straight-line points) or <path> (female, curved
// d attribute) — the two source datasets use different keys (svgPoints
// vs. paths) precisely so this can tell which shape to make without a
// separate flag.
function createShape(muscleId, shapeData, isPath, fill, onClick, stats) {
  const el = document.createElementNS(SVG_NS, isPath ? 'path' : 'polygon');
  el.setAttribute(isPath ? 'd' : 'points', shapeData.trim());
  el.style.cursor = 'pointer';
  el.style.fill = fill;
  if (onClick) {
    el.addEventListener('click', () => onClick({ muscle: muscleId, data: stats[muscleId] }));
  }
  return el;
}
...
function applyInlineStyle(el, style) {
  ...
  Object.entries(style).forEach(([key, value]) => {
    ...
    const cssProp = key.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`).replace(/^ms-/, '-ms-');
...
function resolveOptions(options = {}) {
  return {
    ...
    type: options.type ?? 'anterior',
    gender: options.gender ?? 'male',
  };
}

function datasetFor(state) {
  const isAnterior = state.type === 'anterior';
  if (state.gender === 'female') {
    return { entries: isAnterior ? FEMALE_ANTERIOR : FEMALE_POSTERIOR, isPath: true };
  }
  return { entries: isAnterior ? MALE_ANTERIOR : MALE_POSTERIOR, isPath: false };
}

export function createBodyHighlighter(options) {
  const state = resolveOptions(options);
  ...
  svg.setAttribute('viewBox', `0 0 ${TARGET_VIEWBOX_WIDTH} ${TARGET_VIEWBOX_HEIGHT}`);

  function render() {
    const stats = aggregateStats(state.data);
    const { entries, isPath } = datasetFor(state);

    const shapes = entries.flatMap((entry) => {
      const fill = colorForMuscle(stats, state.highlightedColors, entry.muscle) ?? state.bodyColor;
      const shapeDataList = isPath ? entry.paths : entry.svgPoints;
      return shapeDataList.map((shapeData) =>
        createShape(entry.muscle, shapeData, isPath, fill, state.onClick, stats)
      );
    });

    // Female geometry needs the per-view transform (see
    // FEMALE_VIEW_TRANSFORMS above) to fit this library's fixed
    // viewBox; male geometry is already natively defined in that space
    // and renders directly, matching the original's behaviour exactly.
    if (state.gender === 'female') {
      const group = document.createElementNS(SVG_NS, 'g');
      group.setAttribute('transform', FEMALE_VIEW_TRANSFORMS[state.type]);
      shapes.forEach((shape) => group.appendChild(shape));
      replaceChildren(svg, [group]);
    } else {
      replaceChildren(svg, shapes);
    }
  }

  function mount() {
    if (state.container && wrapper.parentElement !== state.container) {
      state.container.appendChild(wrapper);
    }
  }

  function applyStyles() {
    applyInlineStyle(wrapper, state.style);
    applyInlineStyle(svg, state.svgStyle);
  }
  ...
  return {
    ...
    update(next) {
      if (!next) return;
      if (next.bodyColor !== undefined) state.bodyColor = next.bodyColor;
      if (next.data !== undefined) state.data = next.data;
      if (next.highlightedColors !== undefined) {
        state.highlightedColors = next.highlightedColors ? [...next.highlightedColors] : [];
      }
      if (next.onClick !== undefined) state.onClick = next.onClick;
      if (next.container !== undefined) {
        state.container = next.container;
        if (state.container) mount();
        else if (wrapper.parentNode) wrapper.parentNode.removeChild(wrapper);
      }
      if (next.wrapperClassName !== undefined) {
        state.wrapperClassName = next.wrapperClassName ?? DEFAULT_WRAPPER_CLASS;
        wrapper.className = state.wrapperClassName;
      }
      if (next.style !== undefined) state.style = next.style;
      if (next.svgStyle !== undefined) state.svgStyle = next.svgStyle;
      if (next.type !== undefined) state.type = next.type;
      if (next.gender !== undefined) state.gender = next.gender;
      applyStyles();
      render();
      if (state.container) mount();
    },
